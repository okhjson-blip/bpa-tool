...
9. 과제별 AX 성과지표 전체 PDF 또는 과제정보 CSV 출력
...
- `GET /api/analysis/project/:projectId/report.csv?task_id=:taskId`: 결과 리포트 전체 항목을 과제당 한 행으로 출력하는 UTF-8 CSV. As-Is/To-Be는 플로우차트 순서의 작업방식·도구·수행시간(h:s). 개발비 열 이름은 `추정 외주개발비`. 상세 양식은 `RESULT_REPORT_CSV_SCHEMA.md`를 따릅니다.
...
- `panel_drafts`는 협력사·사용자 계정·패널·화면 범위별 최신 임시 저장본 한 건을 보관합니다. 소유자는 세션마다 바뀔 수 있는 Auth 사용자가 아니라 이메일 기준 `company_user_accounts.id`이므로 재접속·기기 변경 후에도 직전 저장본을 불러옵니다. 자동 저장은 하지 않으며 정식 등록 또는 분석 저장이 완료되면 해당 임시 저장본을 삭제합니다. BDW 태그는 기존 즉시 저장을 유지하고 AI API Key는 임시 저장 대상에서 제외합니다.
- 임시 저장은 과제의 `current_step`을 앞으로만 이동시킵니다. 뒤 단계까지 진행한 과제에서 앞 단계를 다시 임시 저장해도 진행 상태가 되돌아가지 않습니다.
- 프로세스 일괄 저장은 Postgres 함수 `sync_task_processes`가 단일 트랜잭션으로 처리합니다. 중간 실패 시 부분 저장이 남지 않고, 임시 저장본이 들고 있던 이미 삭제된 프로세스 ID는 오류 대신 신규 행으로 되살립니다.
...
- 결과 페이지는 PDF 프리뷰를 먼저 표시합니다. PDF에는 과제 참여자만 이름·직급·역할·이메일로 출력하고, BDW 요약·전체 목록과 L6 기준 AS-IS·To-Be 작업방식·도구를 포함합니다. 사용자가 `결과 리포트 저장`을 누른 경우에만 `task_reports` 최신 PDF 리포트 스냅샷을 저장하며, 관리자 과제 목록의 `상세 조회` 버튼이 활성화됩니다. 과제정보 CSV는 과제당 한 행으로 결과 리포트 전체 항목을 출력합니다.
...
AI Draft, BDW, AI FIT처럼 외부 모델을 호출하는 함수의 제한시간은 `vercel.json`에서 300초로 설정되어 있습니다. 사용하는 Vercel 요금제의 함수 실행시간 한도도 함께 확인해야 합니다.
