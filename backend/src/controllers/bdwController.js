...
export const createToBe = async (req, res) => {
  const { projectId } = req.params;
  const { taskId, ai_analysis } = req.body;
  ...
  try {
    ...
    const toBeProcesses = await Promise.all(processes.map(async (proc) => {
      const analysis = ai_analysis.find((item) => Number(item.process_id) === Number(proc.id));
...
export const generateReport = async (req, res) => {
  ...
  try {
    ...
    const annualFrequency = parseInt(req.query.annual_frequency || '52');
    const annualSavingsHours = (timeSavings * annualFrequency) / 60;
    const fte = annualSavingsHours / 2248;
    const automatedCount = aiAnalysis.filter((analysis) => analysis.fit_category === 'A').length;
    ...
    const reportToBeProcesses = processes.map((process) => {
      const toBe = toBeByProcessId.get(Number(process.id));
      const analysis = analysisByProcessId.get(Number(process.id));
      return {
        original_process_id: process.id,
        name: toBe?.name || process.name,
        ai_applied: toBe?.ai_applied ?? (analysis?.fit_category === 'A'),
        ...
        automation_method: toBe?.automation_method || analysis?.recommended_tech || 'manual'
      };
    });
