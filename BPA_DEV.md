...
16. 임시 저장은 자동 실행하지 않고 패널별 `저장` 버튼에서만 수행합니다. 범위는 프로젝트 기본정보, 과제 기본정보, 인터뷰 답변, 프로세스 수정, 리포트 수행 빈도이며 재접속 시 최신 저장본을 자동 로딩합니다. 정식 등록·AI Draft 저장·프로세스 동기화·결과 리포트 저장이 끝나면 해당 임시 저장본을 삭제합니다.
